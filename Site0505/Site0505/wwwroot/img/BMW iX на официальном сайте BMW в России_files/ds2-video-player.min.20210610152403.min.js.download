    ...
    function ($, LazyLoad, Messages, _, CookieController, trackingVideoLayer, trackingVideoContainer, consentcontroller, DS2VideoPlayerAccessibility) {
        ...
        proto._create = function () {
            ...
            if (this.context.layerLink) {
                ...
                $(window).on('ds2-consentChanged', function (event) {
                    if (consentcontroller.api.isRegulationAccepted() && self.options.lastAction === 'fallbackOpen') {
                        self._privacyCheck();
                    }
                    if (consentcontroller.api.isRegulationAccepted() && self.options.isAutoloop && self.$element.hasClass('ds2-video-player-auto-loop-fallback')) {
                        self._autoloop();
                    }
                });
            }
            ...
            if (consentcontroller && consentcontroller.api) {
                consentcontroller.api.addEventListener('consentcontroller.api.initialized', function (event) {
        ...
        proto._autoloop = function () {
            ...
            if(this.options.canAutoplay === true
                && consentcontroller.api.isBrowserPersistenceEnabled(consentcontroller.api.ENUM_PERSISTENCE_TYPE.COOKIE)
                && consentcontroller.api.isRegulationAccepted() === true) {
        ...
        proto._privacyCheck = function () {
            ...
            consentcontroller.api.addEventListener('consentcontroller.api.initialized', function (event) {
                if (consentcontroller.api.isBrowserPersistenceEnabled(consentcontroller.api.ENUM_PERSISTENCE_TYPE.COOKIE)) {
                    if (consentcontroller.api.isRegulationAccepted() === true) {
                        ...
                        if (self.options.openedInLayerId) {
                            Messages.showCookieDisclaimer($('[data-lightbox-id=' + self.options.openedInLayerId + ']'));
                        } else {
                            Messages.showCookieDisclaimer();
                        }
                    }
                } else {
                    Messages.showCookieBrowserDisabled();
                }
            });
        };
